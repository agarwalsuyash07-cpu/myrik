#!/usr/bin/env python3
"""
Local web frontend for the order-report scripts.
...
them in a subprocess and its stdout comes back to the page. Any workbook the
script writes shows up as a download link.
"""
...
PORT = 8765
TIMEOUT = 300
RUNS = {}  # token -> workdir, so /dl can serve generated files
...
def run_script(fields, files):
    ...
    workdir = tempfile.mkdtemp(prefix="myrik_")
    RUNS[token] = workdir
    ...
    proc = subprocess.run(
        [sys.executable, os.path.join(HERE, script)] + build_args(main, fields),
    ...
    return {
        ...
        "files": [{"name": n, "url": f"/dl?t={token}&f={urllib.parse.quote(n)}"} for n in new],
    }
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def download(self):
        ...
        workdir = RUNS.get(q.get("t", [""])[0])
        name = os.path.basename(q.get("f", [""])[0])
        path = os.path.join(workdir or "", name)
        if not workdir or not name or not os.path.isfile(path):
            return self._send(404, b'{"error":"expired"}')
    ...
    def do_POST(self):
        ...
        try:
            ...
            result = {"ok": False, "stdout": "", "stderr": f"Timed out after {TIMEOUT}s.", "files": [], "code": None}
